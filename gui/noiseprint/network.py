...
class FullConvNet(object):
  ...
  def _batch_norm(self, x, name = 'bnorm'):
    ...
    with tf.compat.v1.variable_scope(name):
      ...
      #       initializer=tf.compat.v1.constant_initializer(self._bnorm_init_var, dtype=tf.float32),
      #       trainable=False)
      moving_mean = tf.compat.v1.get_variable(
        'moving_mean', params_shape, tf.float32,
      ...
      moving_variance = tf.compat.v1.get_variable(
        'moving_variance', params_shape, tf.float32,
        initializer=tf.compat.v1.constant_initializer(self._bnorm_init_var),
        trainable=False)
      ...
      #     initializer=tf.compat.v1.random_normal_initializer(stddev=self._bnorm_init_gamma, dtype=tf.float32))
      gamma = tf.compat.v1.get_variable(
        'gamma', params_shape, tf.float32,
        initializer=tf.compat.v1.random_normal_initializer(stddev=self._bnorm_init_gamma))
      self.variables_list.append(gamma)
      self.trainable_list.append(gamma)

      local_mean, local_variance = tf.nn.moments(x=x, axes=[0, 1, 2], name='moments')

      mean, variance = tf.cond(
        pred=self.falg_train, true_fn=lambda: (local_mean, local_variance),
  ...
  def _conv(self, x, filter_size, out_filters, stride, name='conv'):
    ...
    with tf.compat.v1.variable_scope(name):
      ...
      y = tf.nn.conv2d(input=x, filters=kernel, strides=[1, stride, stride, 1], padding=self.padding)
    return y
