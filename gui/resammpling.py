...
from PySide6.QtWidgets import QWidget, QDoubleSpinBox, QVBoxLayout, QSlider, QHBoxLayout, QLabel, QSpinBox, QCheckBox, QPushButton, QGridLayout, QSizePolicy, QScrollArea
...
class ResamplingWidget(ToolWidget):
    ...
    def __init__(self, filename, image, parent=None):
        ...
        top_layout.addWidget(QLabel(self.tr("A 5x5 probability filter might be able to uncover more complex interpolation algorithms, but the computing time for the probability maps increases significantly.\n" +
                                "Left mouse button: choose location to construct area of interest\n" +
                                "Right mouse button: delete last location.")),1,0)

        top_layout.addWidget(QLabel(self.tr("If no area of interest is chosen for probability, the probability map for the entire image will be calculated.\n" +
        ...
        self.four_parameters_label = QLabel("Fourier map options: (Make sure to have Hanning or R. I. W. checked, same for Highpass 1 or 2.)")
        ...
        if len(self.selected_points_probability) == 0:
            ...
            for i in range(0,len(self.selected_points_probability),2):
                if i + 1 < len(self.selected_points_probability):
                    ...
                    if self.filter_5x5_Check.isChecked():
                        processed_part = self.calculate_probability_map_5x5(self.imagegray_nomalized_copy[min(y1, y2):max(y1, y2)+1, min(x1, x2):max(x1, x2)+1])
                        self.probability_maps.append(processed_part)
                        self.imagegray_copy_for_probabilitymaps[min(y1, y2)+2:max(y1, y2)-1, min(x1, x2)+2:max(x1, x2)-1] = processed_part
                    else:
                        processed_part = self.calculate_probability_map_3x3(self.imagegray_nomalized_copy[min(y1, y2):max(y1, y2)+1, min(x1, x2):max(x1, x2)+1])
                        self.probability_maps.append(processed_part)
                        self.imagegray_copy_for_probabilitymaps[min(y1, y2)+1:max(y1, y2), min(x1, x2)+1:max(x1, x2)] = processed_part
    ...
    def calculate_fourier_maps(self):
        ...
        for prob_map in self.probability_maps:
            if(len(self.probability_maps) == 1 and len(self.selected_points_probability) == 0):
                ...
                self.userfourplot[min(y1,y2):max(y1,y2)+1,[x1,(x1+1),(x1-1),(x1-2),(x1+2),x2,(x2+1),(x2-1),(x2-2),(x2+2)]]=0 #y-stripes
                self.userfourplot[[y1,(y1-1),(y1+1),(y1-2),(y1+2),y2,(y2-1),(y2+1),(y2-2),(y2+2)],min(x1,x2):max(x1,x2)+1 ]=0 #x-stripes
        ...
        for j in range(0,len(self.selected_points_fourier),2):
            if j + 1 < len(self.selected_points_fourier):
                ...
                processed_part = self.calculate_fourier_map(self.imagegray_copy_for_probabilitymaps[min(y1, y2):max(y1, y2)+1, min(x1, x2):max(x1, x2)+1])
    ...
    def calculate_fourier_map(self, process_prob_map):
        ...
        if self.center_four_check.isChecked():
            ...
            half_size = (width // 4)
            fourier_center = fourier[center_y - half_size:center_y + half_size, center_x - half_size:center_x + half_size]
        else:
    ...
    def draw_rectangle(self, p1, p2):
        ...
        (x2,y2) = p2
        if self.probability_check.isChecked():
            self.imagegray[min(y1,y2):max(y1,y2)+1,[x1,(x1+1),(x1-1),x2,(x2+1),(x2-1)]]=0 #y-stripes
            self.imagegray[[y1,(y1-1),(y1+1),y2,(y2-1),(y2+1)],min(x1,x2):max(x1,x2)+1 ]=0 #x-stripes
        elif self.fourier_check.isChecked():
            self.userfourplot[min(y1,y2):max(y1,y2)+1,[x1,(x1+1),(x1-1),x2,(x2+1),(x2-1)]]=0 #y-stripes
            self.userfourplot[[y1,(y1-1),(y1+1),y2,(y2-1),(y2+1)],min(x1,x2):max(x1,x2)+1 ]=0 #x-stripes
    ...
    def build_matrices_for_processing_5x5(self, I):
        k = 0
        F = np.zeros(((I.shape[0] - 4) * (I.shape[1] - 4) ,24)) #the i^th row of the matrix F corresponds to the pixel neighbors of the i^th element of f
