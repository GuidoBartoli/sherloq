...
class DigestWidget(ToolWidget):
    def __init__(self, filename, image, parent=None):
        ...
        table.append([None, self.tr('Block mean'), str(cv.img_hash.blockMeanHash(image)[0].tolist())])
        ...
        table.append([None, self.tr('Perceptual'), str(cv.img_hash.pHash(image)[0].tolist())])
